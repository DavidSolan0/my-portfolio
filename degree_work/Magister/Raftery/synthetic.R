...
base::source   (paste0(loc,'rfunctions2.R'))
...
for (i in 1:(n-1)) {
   for (j in (i+1):n) {
      if (i %in% samples_pilot[[j]]) {
         ...
         samples_pilot[[j]] <- samples_pilot[[j]][which(samples_pilot[[j]] != i)]
      }
   }
...
for (i in 1:(n-1)) {
   ...
   for (b in 1:n_sams) {
      tmp <- NULL
      for (h in 1:H) {
         tmp[h] <- abs(loglik_hat_ih_R (i, sai0 = sa0[[i]][st0[[i]] == st[h]], efi0 = ef0[[i]][st0[[i]] == st[h]], p, m, mu = out_mu[b,], alpha = out_alpha[b,], sig2 = out_sig2[b,], W, X))
      }
      if (sum(tmp) > 0) {
   ...
   if (i%%floor(0.1*n) == 0) cat("Working on actor ", i, " out of ", n, "\n", sep = "")
   rm(st, H, wi, tmp)
}   
...
for (i in 1:(n-1)) {
   ...
   for (j in 1:length(ni)) {
      h   <- ni[j]
      tmp <- ((i+1):n)[which(stratum_labels[[i]] == h)]
      si  <- c(si, tmp[sample.int(n = length(tmp), size = sample_sizes[[i]][j], replace = F)])
   }
...
for (i in 1:p)
   hist_plot(x = out_sig2[,i],  main = bquote(sigma[.(i)]^2))
for (i in 1:p)
